...
import { soglieDiTipo } from '../runtime/soglie.js'
...
export async function run(service, ctx) {
  ...
  // Le soglie dichiarate in config per QUESTO tipo di risorsa, con sopra quelle del suo ACCOUNT
  // (`soglie: { perAccount: { staging: … } }`). Il provider le fonde sui propri default, e
  // `cfg.soglie` del singolo servizio vince su entrambe: i modelli Bedrock sono autoscoperti, quindi
  // senza il livello per tipo non ci sarebbe nessun posto dove tararli.
  const extra = {
    scheduleState: ctx?.tf?.schedules?.[cfg.function],
    t,
    soglie: soglieDiTipo(ctx?.soglie, cfg.type, service.account),
  }
